...
class BatchGenerator(object):
    ...
    def save_pseudo_labels(self, save_dir):
        labels = np.save(os.path.join(save_dir, 'pseudo_labels.npy'), self.pseudo_labels, allow_pickle=True)
    ...
    def next_batch(self, batch_size):
        ...
        return batch_input_tensor, batch_target_tensor, batch_pseudo_tensor, mask
    ...
    def update_pseudo_labels(self, pseudo_labels, uncertainty_scores, thres):
        ...
        for b, sample in enumerate(batch):
            ...
            for i in range(len(timestamp)):
                ...
                '''if i > 0:
                    min_length = max(10, int(0.1 * (timestamp[i] - timestamp[i-1])))
                    max_length = max(20, int(0.2 * (timestamp[i] - timestamp[i-1])))
